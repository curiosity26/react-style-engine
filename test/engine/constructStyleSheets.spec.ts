...
describe('constructStyleSheets', () => {
  ...
  it('constructs stylesheet using child selectors', () => {
    ...
    const [ rule1 ] = Array.from(stylesheet.cssRules)
  ...
  it('constructs stylesheet with :host appended selector', () => {
    ...
    expect(stylesheet).toBeInstanceOf(CSSStyleSheet)
    expect(stylesheet.cssRules).toHaveLength(5)

    const [ rule1, rule2, rule3, rule4, rule5 ] = Array.from(stylesheet.cssRules)
  ...
  it('constructs a stylesheet with keyframe', () => {
    ...
    const [ keyframeRule ] = Array.from(stylesheet.cssRules)
  ...
  it('constructs stylesheets with multiple scales', () => {
    ...
    const [ defaultRule ] = Array.from(defaultStyle.cssRules)
    ...
    expect(desktopStyle.cssRules).toHaveLength(1)
    expect(desktopStyle.media).toEqual('(min-width: 1024px)')

    const [ desktopRule ] = Array.from(desktopStyle.cssRules)
    ...
    const [ hiresRule ] = Array.from(hiresStyle.cssRules)
